(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".result__slider")) new swiper_core_Swiper(".result__slider", {
        ...
        if (document.querySelector(".about__slider")) {
            ...
            const initAboutSlider = () => {
                if (window.innerWidth <= 991.98) {
                    if (!aboutSlider) aboutSlider = new swiper_core_Swiper(".about__slider", {
                        modules: [ Navigation, Pagination ],
                        observer: true,
                        observeParents: true,
                        slidesPerView: 2,
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        window.addEventListener("scroll", (function() {
            ...
            if (currentScroll > lastScrollTop) header.classList.add("header--hidden"); else header.classList.remove("header--hidden");
        ...
        imageElements.forEach((imageElement => {
            ...
            function playFrames() {
                interval = setInterval((() => {
                    if (isPaused) return;
                    imageElement.src = `img/cube/cube_blue/${currentFrame}.webp`;
                    if (pauseFrames.includes(currentFrame)) {
                        isPaused = true;
                        setTimeout((() => {
                            isPaused = false;
                        }), pauseDuration);
                    }
                    if (currentFrame === totalFrames && direction === 1) direction = -1; else if (currentFrame === 1 && direction === -1) direction = 1;
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const slowFactor = .9;
        let currentImageIndex = totalImages;
        const imageElementForScroll = document.querySelector(".services__picture");
        const servicesContent = document.querySelector(".services__content");
        if (imageElementForScroll) {
            function changeImageBasedOnScroll() {
                const rect = servicesContent.getBoundingClientRect();
                const contentHeight = servicesContent.offsetHeight;
                const scrollPercentage = Math.min(Math.max(0, (window.innerHeight - rect.top) / (window.innerHeight + contentHeight)), 1);
                currentImageIndex = totalImages - Math.min(Math.floor(scrollPercentage * totalImages / slowFactor), totalImages - 1);
                const newImageSrc = `img/cube/cube_blue/${currentImageIndex}.webp`;
                imageElementForScroll.setAttribute("src", newImageSrc);
            }
            window.addEventListener("scroll", changeImageBasedOnScroll);
            changeImageBasedOnScroll();
        }
    }));
    document.addEventListener("DOMContentLoaded", (function() {
    ...
    class CustomSlider {
        constructor(container) {
            ...
            this.sliderWrapper = this.container.querySelector(".actions-contact__wrapper");
            this.slides = Array.from(this.container.querySelectorAll(".actions-contact__slide"));
            this.prevButton = this.container.querySelector(".slider__arrow--prev");
            this.nextButton = this.container.querySelector(".slider__arrow--next");
            this.currentIndex = 0;
            this.startX = 0;
        ...
        moveToPrevSlide() {
            ...
            } while (!this.isSlideVisible(this.currentIndex));
            this.updateSlider();
        }
        moveToNextSlide() {
            do {
        ...
        updateSlider() {
            const visibleSlides = this.slides.filter((slide => this.isElementVisible(slide)));
            if (visibleSlides.length === 0) return;
            const currentSlide = visibleSlides[this.currentIndex];
            const slideIndex = this.slides.indexOf(currentSlide);
        ...
        isSlideVisible(index) {
            ...
            return this.isElementVisible(slide);
        }
        onTouchStart(e) {
            this.isSwiping = true;
        ...
        onTouchEnd(e) {
            ...
            if (Math.abs(diffX) > 50) if (diffX > 0) this.moveToNextSlide(); else this.moveToPrevSlide();
            this.isSwiping = false;
        }
    }
    const sliders = document.querySelectorAll(".actions-contact__item");
